...
| Routing | **expo-router** — routes in `src/app/` (`Stack`, single `index` screen) |
...
├── src/
│   ├── app/              # File-based routes (index, _layout)
│   ├── components/       # UI (themed-text, themed-view)
...
- After feature changes, sync docs via `.claude/skills/update-docs-on-code-change/`
